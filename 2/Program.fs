...
let test2 = 
    let iter i =
        ...
        for j = 0 to input.[i].Length - 1 do
            for k = 0 to input.[i].Length - 1 do
                if j <> k && (input.[i].[j] % input.[i].[k]) = 0 then
                    r <- input.[i].[j] / input.[i].[k]
        r
               
    [0..input.Length - 1] |> Seq.sumBy (fun i -> iter i)

[<EntryPoint>]
let main argv = 
    printfn "%A" ([0..input.Length - 1] |> Seq.sumBy (fun i -> Array.max input.[i] - Array.min input.[i]))
    printfn "%A" test2
    0 